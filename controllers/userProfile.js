...
const refreshToken = async (req, res, next) => {
  ...
  const expiresIn = new_data.expires_in - 240;

  query = `match ( user:USER{access_token:'${access_token}'}) 
    SET user.access_token = ${new_data.access_token},user.expires_in = ${expires_in},
